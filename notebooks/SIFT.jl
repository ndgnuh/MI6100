    ...
    for o in 1:num_octaves
        ...
        for layer in 2:(sift.n_octave_layers + 1)
            ...
            for row in (1 + border_width):(height - border_width),
                ...
                pcurve = [hess[x, y, z] for hess in hesses[1:2, 1:2]]
                trH = tr(pcurve)
                detH = det(pcurve)
...
function localize_keypoints!(sift::SIFT, dog_pyramid, keypoints; num_attempts=10)
    ...
    GH = map(eachrow(dog_pyramid)) do dog
        ...
        G = im_gradients(cdog)
        H = im_hessians(cdog)
        Hs = [getindex.((h,), CartesianIndices(H[1])) for h in H]
        Gs = [getindex.((g,), CartesianIndices(G[1])) for g in G]
        return Gs, Hs
    end
    ...
    for keypoint in keypoints
        ...
        ur, uc, us = update
        contrast = abs(D[r, c] + transpose(G[r, c, s]) * update * 0.5)
        if (contrast * sift.n_octave_layers) < sift.contrast_threshold
            continue
        end
